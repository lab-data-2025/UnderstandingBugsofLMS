...
df=pd.read_csv('Sakai_embedding_dimensions256.csv')
...
def find_best_k(data, max_k=60):
    ...
    plt.grid(True)
#     plt.savefig('DBI-Sakai.pdf', format='pdf')  # 或者使用 .svg 文件格式
    plt.show()
...
# 找到最佳的 k 值
reduced_embeddings = TSNE(n_components=2, random_state=42).fit_transform(matrix)
...
plt.savefig('DBI-Sakai-5.pdf', format='pdf', bbox_inches='tight')  # 或者使用 .svg 文件格式
plt.show()
...
df2 = df2.astype(str)
df2 = df2.fillna('')
with pd.ExcelWriter('Sakai_OpenAI_BugCluter52_dim256.xlsx') as writer:
    df2.to_excel(writer, sheet_name='ClusterResult', index=False)
    label_counts_df.to_excel(writer, sheet_name='LabelCounts52', index=False)
    topic_labels_df.to_excel(writer, sheet_name='LabelTopic52', index=False)
