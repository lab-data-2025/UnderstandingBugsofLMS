...
filtered_df = df_bugs[df_bugs['created'] > '2014-12-29']
...
df_commits = df_commits[df_commits['created'] >= '2014-12-29']
# print('数据条数：', df_commits.shape[0])
...
df_cluster = pd.read_csv('Sakai_BugCluster_Type16.csv')
...
category_counts = merged_df['ClusterConvers'].value_counts()

# 计算每个类别的百分比
category_percentage = merged_df['ClusterConvers'].value_counts(normalize=True) * 100
...
result.to_excel('F:/Download/result/sakai_result_1.26.xlsx', index=False)
...
df_filtered = merged_df[(merged_df['LOCM'] <= 5000) & (merged_df['NOFM'] <= 100)]
...
with pd.ExcelWriter('sakai_output_Indicators.xlsx') as writer:
